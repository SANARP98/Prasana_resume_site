...
// Lazy load the CV dropdown - only loads when component mounts
const CVDropdown = dynamic(() => import("./CVDropdown"), {
  ssr: false, // Don't render on server - only client-side
  loading: () => (
    <button className="px-4 py-1.5 bg-darkGray text-cream rounded-md uppercase text-xs font-semibold">
      CV
    </button>
  )
});
...
function SocialLink({
  ...
  return (
    <motion.a
      ...
      className="w-8 h-8 flex items-center justify-center rounded-full border border-darkGray/20 hover:border-accent hover:text-accent transition-colors"
      aria-label={label}
